...
for product in product_items:
    ...
    title_tag = product.find('h6', class_='product-name')  # Find the h6 with class 'product-name'
    link_tag = title_tag.find('a') if title_tag else None  # Find the <a> inside it
    ...
    if product_link != 'No link available':
        ...
        for slider_div in slider_divs:
            ...
            if img_tag and img_tag.has_attr('src'):
                ...
                # Normalize the image URL to HTTPS
                normalized_url = normalize_image_url(image_url)
                slider_images.append(normalized_url)
    ...
    products_data.append({
        'Categories': category,
        'Name': title,
        'Images': ", ".join(slider_images),  # Store multiple images as a comma-separated string
        # 'Regular price': price,
